...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public void uploadDb() {
        ...
        while (cursor.moveToNext()) {
            String english = emptyIfMinus(KEY_ENG);
            String russian = emptyIfMinus(KEY_RUS);
            String transcription = emptyIfMinus(KEY_TRANS);
            String favor = emptyIfMinus(KEY_FAVORITE);
            Integer date = Integer.parseInt(emptyIfMinus(KEY_DATE));
            String id = emptyIfMinus(KEY_ROWID);
   ...
   private String emptyIfMinus(String index){
        int engColumnIndex = cursor.getColumnIndex(index);
        return engColumnIndex >= 0 ? cursor.getString(engColumnIndex) : "";
    }
